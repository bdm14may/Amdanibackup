...
export class UserserviceProvider {
 ...
 getlanguage():Observable<any>{
     let url=null;
     if(localStorage.getItem('Languagef')=='English'){
        url="assets/en.json";
     }else if(localStorage.getItem('Languagef')=='Hindi'){
        url="assets/hn.json";
     }else{
        url="assets/en.json";
     }
 ...
 Sentuser(body): Observable<Iuser> {
     ...
     localStorage.setItem("Languagef", body.Language);
     let a = {
         "Phone_no": id,
         "name": body.name,
         "Image": body.image,
    ...
    Sentuser1(body): Observable<Iuser> {
       ...
       Sentuser2(body): Observable<Iuser> {
         ...
         let bodyString = JSON.stringify(a); 
       
         let headers = new Headers({ 'Content-Type': 'application/json'}); // ... Set content type to JSON
         let options = new RequestOptions({ headers: headers });
         return this._http.post('http://139.59.64.195:3000/api/updateuserlang', bodyString, options)
             .map((res: Response) => { console.log(res);return res.json()})
             .catch((error: any) => Observable.throw(error.json().error || 'Server error'));
        }
 
fetchcropimagefromserver(name){
    return this._http.get(`assets/${name}.json `)
    .map((res: Response) => res.json())
    .catch((error: any) => Observable.throw(error.json().error || 'Server error'));
...
savegooglestate(){
    return new Promise((resolve, reject) => {
        this._http.get(`http://139.59.64.195:6464`).subscribe((res)=>{
            resolve(res);
        },
        ((e)=>{
            reject(e);
...
loaddummymessage(a,v){
    return new Promise((resolve, reject) => {
        this._http.get(`http://139.59.64.195:7414/api/calculatedata/${a}/${localStorage.getItem("Crop_adv")}`).subscribe((res)=>{
            resolve(res.json().result);
            this.savegooglestate();
        },
        ((e)=>{
            reject(e);
        })
    ...
    userUpdate(data): Observable<Ivariety[]> {
        return this._http.post(`http://139.59.64.195:7404/updateuserbyemail`, data)
            .map((res: Response) => res.json())
            .catch((error: any) => Observable.throw(error.json().error || 'Server error'));
    }
