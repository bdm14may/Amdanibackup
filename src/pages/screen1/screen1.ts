...
export class Screen1Page {
    ...
    showdate: any;
    constructor(public navCtrl: NavController, public navParams: NavParams, public modalCtrl: ModalController, private screenOrientation: ScreenOrientation, private _ViewController: ViewController, private el: ElementRef, private _UserserviceProvider: UserserviceProvider, private _DomSanitizer: DomSanitizer, private _ToastService: ToastService, public toastCtrl: ToastController, public tts: TextToSpeech, private _Platform: Platform, public alertCtrl: AlertController, private NativeGeocoder: NativeGeocoder, private _DsortingPipe: DsortingPipe, private _NativeAudio: NativeAudio, private Calendar: Calendar,private file:File,private transfer: FileTransfer,private zip: Zip) {
        this.one = 1;
        
        this.filenamer='http://139.59.64.195/amdanizip/';
        this.count = 0;
        this.zoom = true;
       
        this.imagebkg = 'day';
        // this.fileTransfer.download('http://139.59.64.195/amdanizip/02_Gram.zip', this.file.dataDirectory + '02_Gram.zip').then((entry) => {
        //     console.log('download complete: ' + entry.toURL(),normalizeURL(this.file.dataDirectory) + '02_Gram/bacterial/Gram_Stage_02.png');
        //     this.zip.unzip(entry.toURL(), this.file.dataDirectory + '02_Gram', (progress) => {
              
        //        this.ziped=Math.round((progress.loaded / progress.total) * 100) 
        //        console.log(this.ziped);
        //        this.imgurl=normalizeURL(this.file.dataDirectory+'02_Gram/bacterial/Gram_Stage_02.png');
...
// this.ziped=100;
    ...
    ionViewWillEnter() {
        ...
        this._UserserviceProvider.loaddummymessage(this.showcropeimage, this.vtvalue).then((res) => {
            ...
            this.totalyield=this.showdate[this.showdate.length -1].predicted_yield_as_per_dcc;
            ...
            for (let k = 0; k < this.showdate.length; k++) {
                ...
                if (this.showdate.length == (k + 1)) {
                    if (!lllk) {
                        this._ToastService.presentClosableToast2(this.trfe.no_activity);
                        //this.indexcv=k;
                    }
                }
            ...
            this._ToastService.presentLoadingHide();
            alert("No data found to your location");
        })
    ...
    setFilteredItems(value) {
        ...
        for (let k = 0; k < this.items.length; k++) {
            ...
            if (diffDays == 0) {
                this.indexcv = k;
                this.items[k].img ? this.img = this.filenamer+this.items[k].img : this.img = null;
                this.items[k].img_zoom ? this.zoomimage = this.filenamer+this.items[k].img_zoom : this.zoomimage = null;
                //alert(this.indexcv);
                ...
                this.items[k].img ? this.img = this.filenamer+this.items[k].img : this.img = null;
                this.items[k].img_zoom ? this.zoomimage = this.filenamer+this.items[k].img_zoom : this.zoomimage = null;
                this._ToastService.presentClosableToast4(this.trfe.no_activity);
                ...
                this.NativeGeocoder.reverseGeocode(Number(localStorage.getItem("L_lat")), Number(localStorage.getItem("L_lang")))
                    .then((result: NativeGeocoderReverseResult) => {
                        ...
                        const alert = this.alertCtrl.create({
                            ...
                            message: `<p><b>${this.trfe.crop_land_name}:</b> ${valuer[ik].Map_name}<br><b>${this.trfe.Initial_yield} :${this.totalyield}  (Kg/ha)<br>
                            ${this.trfe.Produce_now} : ${this.totalyield} (Kg/ha)</b><br> <b> ${this.trfe.land_area}:</b> ${valuer[ik].Size} HA </p><p><b>${this.trfe.sowing_date}:</b> ${day2}    ${monthNames[monthIndex2]}  ${year2}</p><p><b>${this.trfe.Harvest_Date}:</b> ${day} ${this.showdate[this.showdate.length -1].sowing_date_cw}</p><br>
<b>${this.trfe.place}:</b> ${this.placeinfo.subLocality?this.placeinfo.subLocality:this.placeinfo.thoroughfare},${this.placeinfo.locality},${this.placeinfo.subAdministrativeArea},${this.placeinfo.administrativeArea}<br>`,
                            buttons: ['Cancel']
                        });
                        alert.present();
                        this._ToastService.presentLoadingHide();
    ...
    showsuggest(r, i): any {
        ...
        if (r.img) {
            r.img ? this.img = this.filenamer+r.img : this.img;
            r.img_zoom ? this.zoomimage = this.filenamer+r.img_zoom : false;
        } else {
            this.img = null;
            this.zoomimage = null;
        }
    ...
    showsuggest1(r, i): any {
        ...
        r.img ? this.img = this.filenamer+r.img : this.img = null;
        r.img_zoom ? this.zoomimage = this.filenamer+r.img_zoom : false;
    ...
    presentToastre(r, i) {
        ...
        this._UserserviceProvider.converttexttohindi(this.items[i].Growth).subscribe(res => {
            ...
            this.startspeech1(this.items[i].Growth);
            this._ToastService.presentLoadingHide();
